...
function showsArticle(data) {
  ...
  for (i = 0; i < data.length; i++) {
    ...
    let articleImg =
      '<div class="thumbnail"> <img class="imagesProd img-responsive" src="' +
      data[i].imageUrl +
      '" alt="l\'image du produit"/> </div>';
    ...
    let articleDescrip =
      '<p class="description">' + data[i].description + '</p>'; //Descriptions des articles
    let articlePrice = '<p class="artPrices">' + data[i].price + ' €' + '</p>'; //Prix de chaque produits
    let btnAddArticle =
      '<p id="local" class="addSelect" value="articleId">Ajouter</p>'; //bouton ajouter produit
    ...
    if (articleId == data[i]._id) {
      ...
      let articleCard =
        '<a class="prodLink" href="./product.html?id=' +
        data[i]._id +
        '">' +
        articleImg +
        articleName +
...
function addArticleInBasket() {
  ...
  let opt =
    document.getElementById('liste').options[
  ...
  for (k = 0; k < panier.length; k++) {
    //boucle dans le tableau panier
    // si cet article est deja present dans le panier et qui a les meme option
    if (
      panier[k].id == articleUrl.searchParams.get('id') &&
      panier[k].option == opt
    ) {
  ...
  if (sameArticle == false) {
    // dans le condition où le boolean est "false"(produit non existent dans panier), on push le produit
    panier.push({
